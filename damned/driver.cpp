...
void Driver::newRace(tCarElt* car, tSituation *s)
{
	...
	float sensorAngles[] = {-90.0, -75.0, -60.0, -45.0, -30.0, -20.0, -15.0, -10.0, -5.0, 0.0, 5.0, 10.0, 15.0, 20.0, 30.0, 45.0, 60.0, 75.0, 90.0};

	for (int i = 0; i < 19; ++i){
		trackSens->setSensor(i, sensorAngles[i], 200.0);
	}

	file.open("/home/student/Documents/torcs-server/torcs-1.3.7-patched/src/drivers/damned/data_damned.csv", ios::app);

	string message("data.csv Found\n");
	cout << message;
...
void Driver::drive(tSituation *s)
{
	...
	if (isStuck()) {
		...
		*/
		...
		// car's outputs (normalised):
		// file << float(car->_accelCmd) << "," << float(car->_brakeCmd) << "," << float(car->_steerCmd) << ",";

		// car's inputs
		float IN_speed = float(car->_speed_x  * 3.6);   // in KPH
		// float IN_dist_to_middle = 2*car->_trkPos.toMiddle/(car->_trkPos.seg->width);
		// // float IN_angle = car->_yaw;
		// float IN_angle =  RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
		// NORM_PI_PI(IN_angle);
		// float IN_rpm = car->_enginerpm*10;

		// Normalise values
		// IN_speed = (MIN(IN_speed, 300)) / 300;

		// file << IN_speed << "," << IN_dist_to_middle << "," << IN_angle << ",";

		// if (IN_dist_to_middle<=1.0 && IN_dist_to_middle >=-1.0 )
		// {
		// 		trackSens->sensors_update();
		// 		for (int i = 0; i < 19; ++i)
		// 		{
		// 				file << (trackSens->getSensorOut(i))/200;
		// 				if (i < 18){
		// 					file << ",";
		// 				}
