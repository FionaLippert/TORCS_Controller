...
static void newRace(int index, tCarElt* car, tSituation *situation)
{
	...
	for (int i = 0; i < 19; ++i){
		trackSens->setSensor(i, -90 + 10.0*i, 200.0);
	}
}
...
static void drive(int index, tCarElt* car, tSituation *situation)
{
	...
	*/
	...
	// Car's inputs:
	float IN_speed = float(car->_speed_x  * 3.6);
	...
	file << IN_speed << "," << IN_dist_to_middle << "," << IN_angle << ",";// << IN_rpm << ",";
